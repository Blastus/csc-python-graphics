...
# Go to BoidGUI for others defined there.
...
def handle_argv(root):
    if len(sys.argv) > 1:
        switch = sys.argv[1]
        if switch.startswith('/s'):
            # /s = screensaver
            global USE_WINDOW, FULLSCREEN, SCR_SAVER, COME_BACK
            # Ensure program is in fullscreen mode.
            USE_WINDOW = False
            FULLSCREEN = True
            # Force going into screensaver mode.
            SCR_SAVER = True
            # Disable having the program come back.
            COME_BACK = -1
        elif switch.startswith('/c'):
            # /c = settings
            root.withdraw()
            tkinter.messagebox.showinfo("Joyce's Screen Saver",
            'This screen saver has no options that you can set.',
            master=root)
            sys.exit()
        elif switch.startswith('/p'):
            # /p = preview
            sys.exit()

def create_boid(root):
...
class BoidGUI(Canvas):
    ...
    # Random Parameters
    MAX_FPS = 30            # Maximum frame per second for display.
    ...
    def check_target(self):
        for group in self.groups:
            ...
            if (group.center - group.target).magnitude <= 100:
                # Adjust target to be over (200) pixels away.
                while (group.center - group.target).magnitude <= 200:
    ...
    def force_wall(self, group, boid, seconds):
        ...
        if boid.position.y < self.WALL_MARGIN:
            ...
            boid.velocity.y -= self.WALL_FORCE * seconds
        # Note:     These walls are currently in use.
        # Observe:  group.add_control(self.force_wall)

    def bounce_wall(self, group, boid, seconds):
        # Note: These walls are not in use.
        # Left and Right walls.
        if boid.position.x < self.WALL_MARGIN:
...
class BoidAgent:
    ...
    def __rule_2(self, boids):
        ...
        for boid in boids:
            ...
            space = (boid.radius + self.radius) * 2
            if delta < space:
    ...
    def __rule_3(self, boids):
        ...
        return (vector - self.velocity) / 50
...
class Vector2:
    ...
    def polar_repr(self):
        x, y = self.x, self.y
        magnitude = hypot(x, y)
        degrees = degrees(atan2(x, y)) % 360
        return 'Polar2({!r}, {!r})'.format(magnitude, degrees)

    # Rich Comparison Methods

    def __lt__(self, obj):
        if isinstance(obj, Vector2):
            x, y, x2, y2 = self.x, self.y, obj.x, obj.y
    ...
    def reflect(self, vec):
        ...
        return Vector2(temp * x2 - x1, temp * y2 - y1)
